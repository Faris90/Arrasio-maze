...
const Collisions = require('./lib/cs/Collisions');
const cds = new Collisions();

// Used in instant kill when collide with rectangular maze wall.
const projectileTypeLookup = {
    'bullet': 'bullet',
    'minion': 'minion',
    'drone': 'drone',
    'swarm': 'swarm',
    'trap': 'trap',
    'block': 'block',
    // Add your own custom types below.
    // For example,
...
const dirtyCheck = (p, r) => {
    return entitiesToAvoid.some(e => {
        const diffX = Math.abs(p.x - e.x);
        const diffY = Math.abs(p.y - e.y);

        return (diffX < r + e.size && diffY < r + e.sizeVert);
    });
};
// ============================================================
...
class Entity {
    constructor(position, master = this) {
        ...
        this.getAABB = (() => {
            ...
            // ============================================================
            // Rectangular maze walls. Might not need this.
            // ============================================================
            let savedSizeVert = 0;
            // ============================================================
            ...
            this.updateAABB = active => { 
                ...
                // ============================================================
                if (this.shape === 200){
                    x1 = this.x - this.realSize;
                    y1 = this.y - this.realSizeVert;
                    x2 = this.x + this.realSize;
                    y2 = this.y + this.realSizeVert;
                }
                // ============================================================
                ...
                // Rectangular maze walls.
                // ============================================================
                let sizeVert = getLongestEdge(x1, y1, x1, y2);
                let sizeVertDiff = savedSizeVert / sizeVert;
                // ============================================================
                ...
                if
                (
                    sizeDiff > Math.SQRT2 || sizeDiff < Math.SQRT1_2
                    || sizeVertDiff > Math.SQRT2 || sizeVertDiff < Math.SQRT1_2
                )
                {
    ...
    define(set) {
        ...
        if (set.SIZE != null) {
            this.SIZE = set.SIZE;
            this.SIZE_VERT = set.SIZE;
            if (this.coreSize == null)
            {
                this.coreSize = this.SIZE;
                this.coreSizeVert = this.SIZE;
            }
            this.boundingWidth = set.SIZE;
        }

        if (set.SIZE_VERT != null) {
            this.SIZE_VERT = set.SIZE_VERT;
            this.coreSizeVert = this.SIZE_VERT;

            this.boundingHeight = set.SIZE_VERT;
        }
    ...
    get sizeVert() {
        if (this.bond == null) {
            return (this.coreSizeVert || this.SIZE_VERT) * (1 + (this.skill.level / 45));
        }

        return this.bond.sizeVert * this.bound.sizeVert;
    }
    // ============================================================
    ...
    // ============================================================
    get realSizeVert() {
        return this.sizeVert * ((Math.abs(this.shape) > lazyRealSizes.length) ? 1 : lazyRealSizes[Math.abs(this.shape)]);
    }
    // ============================================================
    ...
    confinementToTheseEarthlyShackles() {
        // ====================================================
        // Typo?
        // if (this.x == null || this.x == null) // Original
        // ====================================================
    ...
    exportDefintionsToClient = (() => { 
        ...
        function getMockup(e, positionInfo) {
            return { 
                ...
                // Rectangular maze walls.
                // ============================================================
                sizeVert: rounder(e.sizeVert),
                realSizeVert: rounder(e.realSizeVert),
                // ============================================================
                ...
                function check(camera, obj)
                {
                    ...
                    if (obj.shape === 200){
                        ...
                        return Math.abs(obj.x - camera.x) < camera.fov * 0.6 + 1.5 * obj.size + 100 &&
                            Math.abs(obj.y - camera.y) < camera.fov * 0.6 * 0.5625 + 1.5 * obj.sizeVert + 100;
                    }
                    else {
                        // 0.5625 = 9 / 16
                        return Math.abs(obj.x - camera.x) < camera.fov * 0.6 + 1.5 * obj.size + 100 &&
                        ...
                        gazeUpon: () => {
                            ...
                            let visible = nearby.map(function mapthevisiblerealm(e) {
                                if (e.photo) {
                                    ...
                                    if (e.shape === 200){
                                        let width = e.size;
                                        let height = e.sizeVert;

                                        if
                                        (
                                            Math.abs(e.x - x) < fov / 2 + 1.5 * width &&
                                            Math.abs(e.y - y) < fov / 2 * (9 / 16) + 1.5 * height
                                        ) {
                                            // Grab the photo
                        ...
                        function flattener() {
                            ...
                            function challenge(value, challenger) {
                                return value[1] === challenger[0] &&
                                    ...
                                    // Rectangular maze walls.
                                    // ============================================================
                                    value[4] === challenger[3] &&
                                    value[5] === challenger[4] &&
                                    value[6] === challenger[5];
                                // ============================================================
                                ...
                                update: (data) => {
                                    ...
                                    data = data.map(d => { 
                                        return [
                                            ...
                                            d[3],
                                            d[4],
                                            // Rotation. It is not recommended because of high CPU usage.
                                            d[5]
                                            // ============================================================
                                        ];
                                    });
                            ...
                            (() => {
                                ...
                                return () => {
                                    ...
                                    let clean = publicmap.update(minimap.map(function(entry) {
                                        ...
                                        // Rectangular maze walls.
                                        // ============================================================
                                        // entry[4] and entry[5] are Maze wall dimensions.
                    ...
                    return () => {
                        ...
                        entities.forEach((my) => {
                            if (my.settings.drawShape && ran.dice(my.stealth * c.STEALTH)) {
                                ...
                                if (i != -1) {
                                    // update position
                                    minimap[i] = [my.id, my.x, my.y, my.color, my.size, my.sizeVert, my.rotation, my.type, my.team];
                                }
                                else {
                                    // add position
                                    minimap.push([my.id, my.x, my.y, my.color, my.size, my.sizeVert, my.rotation, my.type, my.team]);
                                }
                                // ============================================================
                            }
                        });
...
var gameloop = (() => {
    ...
    let collide = (() => {
        ...
        // Rectangular maze walls.
        // =============================================================================================
        function wallCollideReflect(wall, tank) {
            // https://yal.cc/rectangle-circle-intersection-test/
            let wallHalfWidth = wall.realSize;
            let wallHalfHeight = wall.realSizeVert;

            // Default is circular body.
            let tankBody = cds.createCircle(tank.x, tank.y, tank.realSize);

            const wallBody = cds.createPolygon(wall.x, wall.y,
                [
                    // These points are relative to start point,
                    // in this case - wall.x and wall.y.
                    [-wallHalfWidth, -wallHalfHeight], // Top-left corner.
                    [wallHalfWidth, -wallHalfHeight], // Top-right corner.
                    [wallHalfWidth, wallHalfHeight], // Bottom-right corner.
                    [-wallHalfWidth, wallHalfHeight]  // Bottom-left corner.
                ]);
            wallBody.angle = wall.rotation;

            const result = cds.createResult();

            if(tankBody.collides(wallBody, result)) {
                // =================================================================
                // Tank collides with wall.
                // Tank is on left side of the wall: overlap_x is positive value.
                // Tank is on right side of the wall: overlap_x is negative value.
                // Tank is on top side of the wall: overlap_y is positive value.
                // Tank is on bottom side of the wall: overlap_y is negative value.
                // =================================================================

                // These two lines of code are not necessary, but just for the safe side.
                tank.x -= (result.overlap * result.overlap_x);
                tank.y -= (result.overlap * result.overlap_y);

                tank.accel.x -= (result.overlap * result.overlap_x);
                tank.accel.y -= (result.overlap * result.overlap_y);
        ...
        function wallCollideInstantKill(wall, projectile) {
            let wallHalfWidth = wall.realSize;
            let wallHalfHeight = wall.realSizeVert;

            const circle = cds.createCircle(projectile.x, projectile.y, projectile.size);

            const polygon = cds.createPolygon(wall.x, wall.y,
                [
                    // These points are relative to start point,
                    // in this case - wall.x and wall.y.
                    [-wallHalfWidth, -wallHalfHeight], // Top-left corner.
            ...
            polygon.angle = wall.rotation;

            const result = cds.createResult();

            if(circle.collides(polygon, result)) {
                projectile.health.amount = 0;
        ...
        return collision => {
            ...
            // Rectangular maze walls.
            // =============================================================================
            // Reflect collide tanks against rectangular maze walls.
            // 3 => Maze Wall
            // 5 => Tank
            // =============================================================================
            else if
            (
                (instance.type === 'mazewall' && (other.type === 'tank' || other.type === 'miniboss')) ||
                (other.type === 'mazewall' && (instance.type === 'tank' || instance.type === 'miniboss'))
            ) {
                if (instance.type === 'mazewall') {
                    wallCollideReflect(instance, other);
                }
                else {
                    wallCollideReflect(other, instance);
                }
            }
            // =============================================================================
            // Rectangular maze walls.
            // =============================================================================
            // Kill bullets, swarms, drones, minions, traps, etc when they hit maze walls.
            // 3 => Maze Wall
            // 4 => Phantom Wall
            // 10 => Food
            // =============================================================================
            ...
            (
                (instance.type === 'mazewall' && projectileTypeLookup[other.type]) ||
                (other.type === 'mazewall' && projectileTypeLookup[instance.type])
            ) {
                if (instance.type === 'mazewall') {
                    wallCollideInstantKill(instance, other);
                }
                else {
                    wallCollideInstantKill(other, instance);
                }
            }
            ...
            // Rectangular maze walls.
            // =============================================================================
            // Reflect collide food against rectangular maze walls.
            ...
            (
                (instance.type === 'mazewall' && other.type === 'food') ||
                (other.type === 'mazewall' && instance.type === 'food')
            ) {
...
var maintainloop = (() => {
    ...
    // =========================================================
    // Rectangular Maze Walls Generator.
    // =========================================================
    const createMazeWall = (pos, width, height) => {
        let wall = new Entity(pos);
        wall.define(Class.mazeObstacle);
        wall.SIZE = width;
        wall.SIZE_VERT = height;
        wall.coreSize = width;
        wall.coreSizeVert = height;
        wall.team = -101;
        wall.protect();
        wall.life();
    ...
    let MazeWallGenerator = class {
        run() {
            // Look at app.js where the following code is:
            // let drawWidth = (ratio * instance.size) - (8 * ratio);
            // Maybe 4 more pixels for two sides x 2 pixels.
            const shrinkSize = 12;

            const baseWidth = room.width * 0.02;
            const baseHeight = room.height * 0.02;
            ...
            // Left-most L-shaped
            {
                ...
                createMazeWall(pos, pos.width, pos.height);
            }

            {
                const pos = {
                    x: baseWidth * 7,
                    y: (baseHeight * 18) - shrinkSize,
                    width: baseWidth * 4,
                    height: baseHeight
                };
                createMazeWall(pos, pos.width, pos.height);
            ...
            // To the immediate right of left-most L-shape.
            {
                const pos = {
                    x: baseWidth * 9,
                    y: baseHeight * 10,
                    width: baseWidth,
                    height: (baseHeight * 4)// - (baseHeight / 2)
                };
                createMazeWall(pos, pos.width, pos.height);
            }

            // To form L-shape with the above.
            {
                const pos = {
                    x: (baseWidth * 16) - (shrinkSize),
                    y: baseHeight * 12,
                    width: baseWidth * 6,
                    height: baseHeight
                };
            ...
            // To the immediate right of the above L-shape.
            {
                const pos = {
                    x: baseWidth * 14,
                    y: baseHeight * 6,
            ...
            // Long horizontal line. To the right of the above.
            {
                const pos = {
                    x: (baseWidth * 26),
                    y: baseHeight * 7,
            ...
            // Vertical line. Perpendicular to the above and touches the center.
            {
                const pos = {
                    x: (baseWidth * 27) + (baseWidth / 2) + (shrinkSize * 2) - (shrinkSize / 4),
                    y: baseHeight * 13 + (shrinkSize / 2),
                    width: baseWidth,
                    height: baseHeight * 5 + (baseHeight / 2) + (shrinkSize * 2) + (shrinkSize / 4)
                };
                createMazeWall(pos, pos.width, pos.height);
            }

            // Horizontal line to the right of the above.
            {
                const pos = {
                    x: (baseWidth * 34),
                    y: baseHeight * 13,
            ...
            // Right below top-left most L-shape. Long horizontal line.
            {
                const pos = {
                    x: (baseWidth * 19) - shrinkSize,
                    y: (baseHeight * 20) - (shrinkSize * 2),
                    width: baseWidth * 5,
                    height: baseHeight
                };
                createMazeWall(pos, pos.width, pos.height);
            ...
            // Near-center vertical line.
            // Long vertical line right below the above.
            {
                const pos = {
                    x: (baseWidth * 19),
                    y: (baseHeight * 26) - (shrinkSize * 3),
                    width: baseWidth,
                    height: baseHeight * 4 - (baseHeight / 2) - (shrinkSize * 2)
                };
                createMazeWall(pos, pos.width, pos.height);
            }

            // Vertical line to the right of the above.
            {
                const pos = {
                    x: (baseWidth * 25) + shrinkSize,
                    y: (baseHeight * 34) + (baseHeight / 2) - (shrinkSize / 5),
                    width: baseWidth,
                    height: baseHeight * 7 + (shrinkSize * 3)
                };
                createMazeWall(pos, pos.width, pos.height);
            }

            // Upper horizontal line to the right of the above.
            {
                const pos = {
                    x: (baseWidth * 31) + shrinkSize,
                    y: (baseHeight * 28),
                    width: baseWidth * 3,
                    height: baseHeight
                };
            ...
            // Lower horizontal line to the right of the above.
            {
                const pos = {
                    x: (baseWidth * 31),
                    y: (baseHeight * 35),
            ...
            // Vertical line above the above.
            {
                const pos = {
                    x: (baseWidth * 35),
                    y: (baseHeight * 29) + shrinkSize,
                    width: baseWidth,
                    height: baseHeight * 5
                };
                createMazeWall(pos, pos.width, pos.height);
            ...
            // Long vertical line to the right of the above.
            {
                const pos = {
                    x: (baseWidth * 40),
                    y: (baseHeight * 18),
            ...
            {
                const pos = {
                    x: (baseWidth * 45) + (baseWidth / 2) + (shrinkSize * 2) - (shrinkSize / 4),
                    y: (baseHeight * 17),
                    width: baseWidth,
                    height: baseHeight * 4
                };
            ...
            // Short horizontal line right above the above.
            {
                const pos = {
                    x: (baseWidth * 43) - shrinkSize,
                    y: (baseHeight * 12) + shrinkSize,
                    width: baseWidth * 2,
                    height: baseHeight
                };
                createMazeWall(pos, pos.width, pos.height);
            ...
            // Short horizontal line forming enclosure.
            {
                const pos = {
                    x: (baseWidth * 43),
                    y: (baseHeight * 24) - shrinkSize,
                    width: baseWidth * 4,
                    height: baseHeight
                };
                createMazeWall(pos, pos.width, pos.height);
            ...
            // Long horizontal line below the above.
            {
                const pos = {
                    x: (baseWidth * 40) + shrinkSize,
                    y: (baseHeight * 40) - shrinkSize,
                    width: baseWidth * 4,
                    height: baseHeight
                };
                createMazeWall(pos, pos.width, pos.height);
            ...
            // Horizontal line to the left of the above.
            {
                const pos = {
                    x: (baseWidth * 13) + shrinkSize,
                    y: (baseHeight * 30) - (shrinkSize * 2),
                    width: baseWidth * 5,
                    height: baseHeight
                };
                createMazeWall(pos, pos.width, pos.height);
            ...
            // Short horizontal line to the above of the above.
            {
                const pos = {
                    x: (baseWidth * 13),// + (baseWidth / 2) + (shrinkSize * 3),
                    y: (baseHeight * 25),
                    width: baseWidth * 2,
                    height: baseHeight
                };
            ...
            // Vertical line to the left of the above.
            {
                const pos = {
                    x: (baseWidth * 7) + (shrinkSize * 2),
                    y: (baseHeight * 28),
                    width: baseWidth,
                    height: baseHeight * 5
                };
            ...
            // Bottom part. Near center long vertical line.
            {
                const pos = {
                    x: (baseWidth * 15) + shrinkSize,
                    y: (baseHeight * 36),
                    width: baseWidth * 5,
                    height: baseHeight
                };
            ...
            {
                const pos = {
                    x: (baseWidth * 11) + (shrinkSize * 2),
                    y: (baseHeight * 40) - (baseHeight / 2) - shrinkSize,
                    width: baseWidth,
                    height: (baseHeight * 3) - (baseHeight / 2)
                };
                createMazeWall(pos, pos.width, pos.height);
            }
            ...
            {
                const pos = {
                    x: (baseWidth * 22) + shrinkSize,
                    y: (baseHeight * 43) - (shrinkSize * 2),
                    width: baseWidth * 10,
                    height: baseHeight
                };
                createMazeWall(pos, pos.width, pos.height);
...

            {
                const pos = {
                    x: (baseWidth * 33),
                    y: (baseHeight * 45) + (baseHeight / 2) + shrinkSize,
                    width: baseWidth,
                    height: baseHeight * 2
                };
                createMazeWall(pos, pos.width, pos.height);
